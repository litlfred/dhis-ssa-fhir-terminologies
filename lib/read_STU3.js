function processRead(dhisScriptContext) {
	...
	this.dhisScriptContext.logInfo("Schema=" + schema);
	this.dhisScriptContext.logInfo("Matches=" + JSON.stringify(matches,null,'\t'));
	this.dhisScriptContext.logInfo(schema + "=" + JSON.stringify(matches[schema],null,'\t'));
	resp = metadata.identifiableObjectToCodeSystem(matches[schema],schema,displayName);
	break;

    case 'categories':
	// VALUE SET: WHAT SHOULD OR SHOULD NOT BE HER.   THESE ARE SUBSETS 
	// IDENTIFIED OF THE CODE SYSTEMS ABOVE OR OF OTHER CODE SYSTEMS
	var schema = baseid;
	var s_id = chunks[1];
	var metadata = new MetadataFHIR_STU3(dhisScriptContext);   
